...
Run data mining from PostgreSQL Data Warehouse
Save mining results to Supabase

Run:
python save_mining_results.py
"""
...
engine_supabase = create_engine(SUPABASE_DB_URL)
...
df['roast_enc'] = le_roast.fit_transform(
    df['Roast Type']
)

df['country_enc'] = le_country.fit_transform(
    df['Country']
)

df['loyalty_enc'] = le_loyalty.fit_transform(
    df['Loyalty Card'].fillna('No')
)
...
cluster_summary['label'] = (
    cluster_summary.apply(label_cluster, axis=1)
)
...
basket = (
    df.groupby(['Order ID', 'Coffee Type'])['Quantity']
    .sum()
    .unstack(fill_value=0)
    .map(lambda x: 1 if x > 0 else 0)
)

records = basket.apply(
...
if len(multi) >= 5:
    ...
    if len(freq) > 0:
        ...
        for _, r in rules.head(6).iterrows():
            ...
            top_rules.append({
                'antecedent':
                    ', '.join(list(r['antecedents'])),

                'consequent':
                    ', '.join(list(r['consequents'])),

                'support':
                    round(float(r['support']), 3),

                'confidence':
                    round(float(r['confidence']), 3),

                'lift':
                    round(float(r['lift']), 3)
            })

# FALLBACK COOCCURRENCE
cooccur = {}
...
print(f"✓ {len(top_rules)} rules found")
print(f"✓ {len(multi)} multi-item orders")
...
feat_imp = [
    ...
    for f, i in sorted(
        zip(X_clf.columns, clf.feature_importances_),
        key=lambda x: -x[1]
    )
]
...
rmse = float(
    np.sqrt(
        mean_squared_error(y_te, y_pred_r)
    )
)

mae = float(
    np.mean(
        np.abs(y_te - y_pred_r)
    )
)
...
rows.append({
    ...
    'metric_value':
        str(results['clustering']['silhouette_score']),
    'detail':
        json.dumps(results['clustering']['elbow'])
})
...
rows.append({
    ...
    'metric_value':
        str(results['association']['rules_found']),
    'detail':
        json.dumps(results['association']['top_rules'])
})
...
rows.append({
    ...
    'metric_value':
        str(results['association']['multi_item_orders']),
    'detail':
        json.dumps(results['association']['top_cooccurrence'])
})
...
rows.append({
    ...
    'metric_key': 'accuracy',
    'metric_value':
        str(results['classification']['accuracy']),
    'detail':
        json.dumps(results['classification']['feature_importances'])
})
...
rows.append({
    ...
    'metric_value':
        str(results['classification']['cv_mean']),
    'detail':
        json.dumps(results['classification']['class_report'])
})
...
rows.append({
    ...
    'metric_key': 'r2_score',
    'metric_value':
        str(results['regression']['r2_score']),
    'detail':
        json.dumps(results['regression']['coefficients'])
})
...
with engine_supabase.connect() as conn:
    ...
    conn.execute(text("""
        DROP TABLE IF EXISTS mining_results;
    """))
...
print("✓ mining_results.json saved")
...
print(
    f"Clustering     : "
    f"{results['clustering']['silhouette_score']}"
)

print(
    f"Association    : "
    f"{results['association']['rules_found']} rules"
)

print(
    f"Classification : "
    f"{results['classification']['accuracy_pct']}%"
)

print(
    f"Regression     : "
    f"{results['regression']['r2_pct']}%"
)

print("\n✅ All mining results saved successfully!")